"""Repro table amort / prix 9149 @ 2026-03-26 vs réf classeur (~6830.099)."""
...
from obligation_amort_schedule import _parse_date_valo, _tenter_table_amort_pour_code, charger_referentiel_et_echeancier
...
def main() -> None:
    ...
    ref, ech = charger_referentiel_et_echeancier(xlsx, ["9149"])
    ...
    tab = _tenter_table_amort_pour_code(
        ...
        ui={"CODE": "9149", "description": "", "Description": ""},
        ref=ref,
        ech=ech,
    ...
    print("prix_somme_flux_actualises:", prix)
    print("debug_pv_excel_wg:", tab.get("debug_pv_excel_wg"))

    rb = tab.get("rows") or []
    for label in ("Flux restant", "durée", "Taux d'actualisation", "Flux actualisé"):
        for r in rb:
            if r.get("label") == label:
                print(label, [r.get("values")])

    print("amort_engine_id:", tab.get("amort_engine_id"))
